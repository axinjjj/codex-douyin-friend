...
- Direct chat images, ordered image posts, and cover-only fallbacks enter the same Codex thread as `localImage` inputs. Text preceding the next media item in a settled inbound run is preserved in that media turn. Direct images prefer a bounded, signature-checked visible data image or trusted CDN source and fall back to a bounded screenshot without relying on background `requestAnimationFrame`. Shared-work detail requests use three attempts, a two-second per-request cutoff, bounded backoff, and a compatible bitrate source before falling back to the React share-card cover with an explicit evidence boundary. Ignored UUID jobs are deleted after each turn and stale jobs older than two hours are cleaned on startup.
