...
- Direct chat images, ordered image posts, and cover-only fallbacks enter the same Codex thread as `localImage` inputs. Text attached to the same media message is preserved in that turn. Direct images prefer a bounded, signature-checked visible data image or trusted CDN source and fall back to a bounded screenshot without relying on background `requestAnimationFrame`. If the signed-in detail endpoint exposes no work body, only the React share-card cover is supplied with an explicit evidence boundary. Ignored UUID jobs are deleted after each turn and stale jobs older than two hours are cleaned on startup.
...
- Live media probes on 2026-09-02 exposed the former compound-message, zero-width direction, background-screenshot, and null-detail failures. After correction, the current real direct-image data payload and share-card cover both completed bounded local acquisition and immediate cleanup; a combined text plus share-card cover also completed a fresh-thread Codex reply and verified Douyin send.
...
- New inbound activity gets a bounded three-round, 750 ms quiet-window settle. A batch containing any bounded number of text messages plus exactly one media message becomes one media turn, matching Douyin's separate DOM entries for “attached text + shared work”; two media messages remain ambiguous and fail closed.
