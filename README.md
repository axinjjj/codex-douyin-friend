...
- 检测到新入站后会等待一个最长 2.25 秒、以 750 毫秒静默为结束条件的短合并窗口。之后按
  消息时间顺序建立最多 12 条的持久队列：媒体前的连续文字会和紧随其后的媒体进入同一个
  Codex turn，相邻媒体逐条处理，末尾纯文字合成最后一个 turn。每条回复发送并校验成功后才
...
- 压缩只会在没有 Codex turn、媒体处理或发送操作的空闲点启动，并等待官方
  ...
  一次原输入重试，不会形成循环或重复发送。
- 原生 `ShareAweme` 视频卡片已经完成真实往返：桥从当前登录态读取有界的兼容播放候选，
  优先通过抖音官方播放入口取得新鲜 CDN 地址；下载先用 1 字节探测总大小，再按 1 MiB Range
  顺序接收并逐段校验位置和总长度。单段失败最多重试 3 次，只有完整分段才写入文件；重试会
  回到官方入口刷新 CDN 地址。之后按时长自适应抽取关键帧，以 `localImage` 输入同一个 Codex
  thread。跳转目标仍必须属于受信任媒体域名。
...
- 直接聊天图片优先解码当前可见、经过格式/签名/大小校验的 PNG/JPEG，或从受信任媒体地址下载；
  ...
  超过 12 张时等距取 12 张。作品详情请求每次最多等待 2 秒，按 250/500 毫秒退避重试三次，
  并尝试兼容码率地址；全部失败后才只把分享卡片封面交给模型，并明确禁止假装看过完整作品。
  分享时附带的聊天文字会与媒体进入同一个 Codex turn，不再降级成纯文字。
