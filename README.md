...
- `http://127.0.0.1:43127` 控制台可查看组件状态、当前模型/思考强度、上下文窗口占比和最近
  ...
  thread ID、聊天正文或人设内容。
- 直接聊天图片优先解码当前可见、经过格式/签名/大小校验的内嵌图片，或从受信任媒体地址下载，
  两者都不可用时才做有界截图，避免后台标签页暂停动画帧导致截图定位超时。分享的图文作品会从当前登录态读取按原顺序排列的图片，
...
- 这些图片路径已通过本地契约测试；2026-09-02 的真实样本暴露了旧版复合消息丢媒体、后台截图
  ...
  验证本地读取和立即清理成功，仍需清除当前安全阻塞断点，再做一次含 Codex 回复的端到端复验。
