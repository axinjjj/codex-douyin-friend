...
代码按这些职责分层：`douyin-inbound-planner.mjs` 只负责消息批次；
...
- 每个新 thread 都有递增代际；旧代异步结果不能在新 thread 上启动 turn。每个入站批次另有一份
  只含哈希、turn ID 和阶段的副作用日志。`turn/start` 返回后先持久化真实 turn ID，再接收完成
  通知；重启可按 ID 取回已完成回复，Enter 后则先验证出站指纹，只恢复未执行的后置动作。没有
  turn ID 的不明启动、无法验证的发送和已经记录尝试的点赞都不会靠猜测重做。
