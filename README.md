...
- 直接聊天图片优先解码当前可见、经过格式/签名/大小校验的 PNG/JPEG，或从受信任媒体地址下载；
  抖音聊天内嵌的 WebP 不直接交给 Codex，而是做有界 PNG 截图。
  新出现的直图只有在图片源与自然尺寸都已就绪后才进入消息断点；启动时可见直图仍在加载则先重试，
  不会把空源、零尺寸的临时 DOM 状态当成一条新的稳定消息。
  分享作品会综合作品详情、`aweme_info` 与有界解析的动态卡片数据：视频走兼容播放源；
  单张图文保留 `image_post` 身份；
...
- 2026-09-08 一张真实直发相册图片暴露了两个同路径问题：图片尚未加载时的空源、零尺寸状态被写入断点，
  且精确图片源读取表达式重复声明局部变量，导致桥在启动 Codex turn 前安全停机。修复后，同一条待处理图片
  仅在前序可见消息完全一致、旧动作仍为未启动 turn 的 `planned`、当前图片已加载并完成本地取证的条件下恢复；
  原持久 thread 随后生成并发送一条回复，本端精确气泡已观察，桥回到 `listening`，媒体临时目录为空。
  这仍只证明 `local-dom-observed`，不等于抖音平台业务 ACK 或对端送达。
