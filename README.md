...
- 旧版 `.MessageItemShareAwemecontainer` 与新版 `.BulletBulletVideocontainer` 分享卡由同一份
  ...
  IHDR 尺寸和浏览器画布像素统计必须一致，整批纯黑帧会被判为解码失败并尝试下一 H.264 候选，
  绝不会作为 `localImage` 交给 Codex。所有完整视频候选都失败且存在可信封面时才明确降级为
  ...
  已登记卡片的真实 React 播放目标。播放动作前会同步安装静音保护，并监听该动作产生的可信
  视频网络响应；捕获到完整源时继续走原有
  视频下载、关键帧和音轨链。若播放器只暴露 `blob:`/MSE，则直接在仍被静音的可见播放器内
