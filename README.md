...
- `http://127.0.0.1:43127` 控制台可查看组件状态、当前模型/思考强度、上下文窗口占比和最近
  ...
  thread ID、聊天正文或人设内容。
- 直接聊天图片会截取当前入站图片区域作为一个 `localImage`；分享的图文作品会从当前登录态
  读取按原顺序排列的图片，超过 12 张时等距取 12 张并明确告诉模型证据边界。两条新路径
  已通过本地契约测试，仍需各用一个真实样本完成当前抖音网页版本的端到端验证。
...
- 聊天图片截图和图文作品图片只进入 `.runtime/image-analysis/`，同样在 Codex turn 后删除；
...
`gpt-5.6-sol / xhigh`，自动发送默认关闭；修改后的模型、思考强度及自动发送开关只保存到被 Git
...
任务名固定为 `CodexDouyinFriendSupervisor`，使用交互式当前用户、最低权限和隐藏后台进程；
