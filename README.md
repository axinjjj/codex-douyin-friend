...
- 无法归类的一条左侧入站只推进为“已跳过”，不会让同批或之后的纯文字、图片和作品一起停住；
  未确认的右侧活动、发送结果、聊天身份或消息边界仍按原有安全门禁停止。处理中断的旧版空文字表情
...
- 未登记的媒体结构会记录一个仅含 SHA-256 结构签名、数量、通用类名提示和属性名的有界诊断，
  ...
  运行时也不会自动改代码。取证已经开始后的身份、资源、断点或发送歧义仍会安全停机。
...
一个聊天；对方切换、输入不完整、出现未确认的右侧消息、本端气泡无法核验，或系统检测到新增消息
缺少 12 条窗口内的可靠边界时会停止，不会盲目重发。不同的窗口外历史可能最终呈现相同的有限哈希
