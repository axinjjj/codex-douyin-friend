...
受信任好友、一个专用抖音账号和一个专用 Edge 配置；人格与工程规则当前由 Codex 的全局/项目
`AGENTS.md` 指令链共同提供，尚未隔离为独立陪伴运行目录。本项目并非抖音或 OpenAI 的官方产品，
网页自动化可能随页面更新而需要适配。
...
7. 启用媒体点赞后，模型只在现有媒体 turn 内决定是否点赞；回复校验成功后才执行原生点赞。
...
- 2026-09-05 使用固定测试正文完成了一次共享视频原生引用实发，确认出站引用结构和正文可被
...
- 检测到新入站后会等待一个最长 2.25 秒、以 750 毫秒静默为结束条件的短合并窗口。之后按
  ...
  推进该项断点；异常或重启会保留尚未完成的哈希队列，不会重发已经确认成功的前项回复。若旧回复
...
- 完整识别为视频、且两侧都能取得唯一作品身份哈希的共享作品批次，发送前会重新确认该批对应的
  ...
  在推进已发送断点前幂等清理精确的旧引用，无法证明所有权时只停止、不关闭用户后来打开的引用。
  普通文字、直接相册图片、图文和评论分享
...
- 陪伴 thread 当前仍以工程根目录作为 `cwd`，因此私人全局 `AGENTS.md` 与仓库工程指令会按 Codex
  发现顺序层叠。启动检查只证明全局文件包含在 `instructionSources`，尚未验证预期来源的精确集合、
  顺序或非敏感摘要；需要独立 runtime cwd 后才能把人设隔离标为完成。
- 抖音网页没有给桥暴露经验证的稳定消息 ID。当前去重与恢复依赖最多 12 条可见 DOM 消息的
  内容指纹、方向和连续重叠边界，不是独立的持久收件箱；长时间离线、页面结构变化或边界消失
  都会安全停机，不能承诺绝不漏收。
- `reply-latest-douyin-message.mjs` 与 `reply-latest-douyin-video.mjs` 是诊断入口，仍使用临时 thread，
  没有主桥的持续 thread、运行锁、action journal 与同一发送能力合同；在统一唯一发送入口前，
  不应把它们用于正式自动发送验收。可选点赞的累计 ordinal 也尚未完成多次恢复专项验证，证据
  不足时必须跳过，不能拖垮已验证的核心回信。
...
聊天对应的 thread，并从持久消息断点继续。停机期间新收到的左侧文本、内置表情、图片、视频或图文卡片
会在恢复后按时间顺序进入同一条持久队列；多条连续文本会合并为一个 turn，多个媒体则逐条
生成和校验回复。处理视频或转换内置表情 WebP 时，专用 Edge 会在后台创建不可见的
...
回复。人设内容不会复制进此仓库。当前正式 thread 的 `cwd` 仍是工程根目录，所以仓库
`AGENTS.md` 也会参与指令链；这项隔离缺口在“尚未解决的发布边界”中保留。
